...
final class GenerateCacheCommand extends AbstractCommand
{
    ...
    {
        $sql = "SELECT node.id, (COUNT(parent.id) - 1) AS level, node.parentId, node.handle, node.pageType
FROM  cms_sitemap AS node,
        cms_sitemap AS parent
WHERE node.nestedLeft BETWEEN parent.nestedLeft AND parent.nestedRight
GROUP BY node.id
